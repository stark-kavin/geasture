...
def doctor_home(request):
    if request.method == "GET":
        ...
        try:
            user_profile = UserProfiles.objects.get(user=request.user)
            if user_profile.role not in ["doctor", "nurse"]:
                return JsonResponse({"message": "Access restricted to doctors & nurses only", "status": "error"}, status=403)
            
            # Get patients linked to the doctor or nurse
            linked_patients = Patient.objects.filter(patient__account=user_profile)
            patients_data = [
                {"id": patient.id, "name": patient.name, "age": patient.age, "alert": patient.alert}
                for patient in linked_patients
            ]
            
            return JsonResponse({
                "message": "Welcome to the doctor home",
                "status": "success",
                "doctor": {
                    "id": user_profile.id,
