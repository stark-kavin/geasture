...
@csrf_exempt
def get_doctor_data_view(request):
    if request.method == "GET":
        if not request.user.is_authenticated:
            return JsonResponse({"message": "Unauthorized", "status": "error"}, status=401)
        try:
            ...
            return JsonResponse({"message": "User profile not found", "status": "error"}, status=404)

    return JsonResponse({"message": "Only GET method allowed"}, status=405)

from django.dispatch import receiver
from .realtime_db import reset_and_insert_data
reset_and_insert_data()

@receiver(post_save, sender=Patient)
@receiver(post_delete, sender=Patient)
def patient_change_handler(sender, instance, **kwargs):
    reset_and_insert_data()

# Signal for PatientLink model
@receiver(post_save, sender=PatientLink)
@receiver(post_delete, sender=PatientLink)
def patientlink_change_handler(sender, instance, **kwargs):
    reset_and_insert_data()